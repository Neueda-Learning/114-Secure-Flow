...
final class AlertSpecifications {
    ...
    static Specification<AlertEntity> withFilters(AlertStatus status, AlertSeverity severity) {
        ...
        if (status != null) {
            result = result.and((root, query, builder) -> builder.equal(root.get("status"), status));
        }
        if (severity != null) {
            result = result.and((root, query, builder) -> builder.equal(root.get("severity"), severity));
        }
        return result;
    }
}
